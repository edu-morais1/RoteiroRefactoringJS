...
function calculaTotalApresentacao(pecas, apre) {
  let total = 0;
...
function calcularTotalFatura(pecas, apresentacoes) {
  ...
  for (let apre of apresentacoes) {
    totalFatura += calculaTotalApresentacao(pecas, apre);
  }
  return totalFatura;
}
...
function gerarFaturaStr (fatura, pecas) {
    ...
    for (let apre of fatura.apresentacoes) {
        faturaStr += `  ${getPeca(pecas, apre).nome}: ${formatarMoeda(calculaTotalApresentacao(pecas, apre))} (${apre.audiencia} assentos)\n`;
    }
...
function gerarFaturaHTML(fatura, pecas) {
    ...
    for (let apre of fatura.apresentacoes) {
        faturaHTML += `<li>  ${getPeca(pecas, apre).nome}: ${formatarMoeda(calculaTotalApresentacao(pecas, apre))} (${apre.audiencia} assentos) </li>\n`;
    }
